        ...
        <div class="container">
            <div class="row">
	            ...
	            @if ($vacancy->draft && $vacancy->moderated)
		            <p style="color: red;">This is a preview mode. Сlick "Publish vacancy" for publishing</p>
	            @elseif($vacancy->draft && !$vacancy->moderated)
					<p style="color: red;">This is a preview mode. To confirm send for moderation, click "Publish vacancy"</p>
	            @endif
	            
                <div class="col-md-9">
                    <div class="manager">
	                    @include('vacancies.partials._showVacancyInfo', ['published' => 'Published', 'deadline' => 'Deadline', 'expired' => 'The deadline for applications has expired ',
	                ...
	                @include('vacancies.partials._showInformation', ['info' => 'Information', 'commonInfo' => 'Common information',
                    ...
                    <div class="sendsms">
		                    ...
		                    <div class="downloadds text-center">
			                    ...
			                    @else
				                    ...
				                    @elseif($vacancy->request_type == 'form_from_file')
					                    ...
					                    <p id="first">{{ $vacancy->form_from_file }}<span>({{ round(filesize(config('admin.imagesUploadDirectory'). '/'. $vacancy->form_from_file)/1000) }}кб)</span><a href="{{ url(config('admin.imagesUploadDirectory'). '/'. $vacancy->form_from_file) }}">Скачать</a></p>
					                    {!! $vacancy->only_in_english ? '<span style="color: red;"">ВНИМАНИЕ! Заявка на данную вакансию должна быть на английском языке</span>' : '' !!}
					                    <a href="" id="send_vacanc" class="response_modal">Откликнуться</a>
				                    @else
	                    ...
	                    @else
		                    <div class="downloadds">
			                    <div class="pull-left">
				                    <a href="{{ route('vacancies.edit', $vacancy->id) }}" id="send_vacanc">Edit</a>
			                    </div>
			                    <div class="pull-right">
				                    <a href="{{ route('vacancies.publishDraft', $vacancy->id) }}" id="send_vacanc">Publish vacancy</a>
			                    </div>
		                    </div>
	                    @endif
                    </div>
                ...
                <div class="col-md-3">
	                @include('vacancies.partials._showCompanyInfo', ['address' => 'Address', 'contactPerson' => 'Contact person', 'phone' => 'Phone'])
			    ...
			    <div class="modal-content">
				    ...
				    <form action="{{ route('vacancy.response.send', ['type' => 'vacancy_form_from_file']) }}" method="POST" enctype="multipart/form-data" class="response_form">
					    <div class="modal-body">
						    ...
						    <label for="" style="padding: 0 10px; text-align: left">Прикрепите файл с заполненной формой:</label>
					    ...
					    <label for="" style="padding: 0 10px">Если необходимо, прикрепите файлы дополнительно:</label>
					    @for($i = 1; $i <= 3; $i++)
						    <div class="form-group" >
							    <div class="form-group b-form-file_upload" style="padding: 0">
								    <label for="file"></label>
			    ...
			    <div class="modal-content">
				    ...
				    <form action="{{ route('vacancy.response.send') }}" method="POST" enctype="multipart/form-data" class="response_form">
					    <div class="modal-body">
						    ...
						    <label for="">Если необходимо, прикрепите файлы дополнительно:</label>
						    @for($i = 1; $i <= 3; $i++)
							    <div class="form-group" style="padding: 0">
								    <div class=" b-form-file_upload">
									    ...
									    <div class="uploadFile one vacansia" data-target="{{ route('ajax.uploadFile', ['section' => 'response_attached_file']) }}" data-token="{{ csrf_token() }}">
										    ...
										    <div class="one-vacansia">
											    <div class="btn btn-primary imageBrowse" style="border-radius: 0!important;"><i class="fa fa-upload"></i>Выберите файл<input class="b-button-upload_video" type="file"></div>
											    <div class="btn btn-danger imageRemove"><i class="fa fa-times"></i> Удалить</div>
										    </div>
										    <input name="file{{ $i }}" class="imageValue" type="hidden" value="">
										    <div class="errors"></div>
